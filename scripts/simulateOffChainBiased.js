...
async function main() {
  ...
  // 🔥 BIAS STRENGTH (adjust for experiments)
  const BIAS_PROB = 0.05; // 5%
  ...
  for (let i = 0; i < TOTAL_GAMES; i++) {
    ...
    // ---------------- 🔥 BIAS INJECTION ----------------
    // If player would have won, sometimes flip outcome
    if (!push && won && Math.random() < BIAS_PROB) {
      if (guessHigher) {
        nextCard = Math.max(1, currentCard - 1); // force lower → loss
      } else {
        nextCard = Math.min(13, currentCard + 1); // force higher → loss
      }
      won = false;
    }
    // --------------------------------------------------
  ...
  console.log(
    `\nStatus: ${close ? "⚠️ Looks fair" : "❌ Clearly biased (deviates from theory)"}`
  );
  console.log("================================================\n");
